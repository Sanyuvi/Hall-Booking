...
app.post("/bookroom/:id", (req, res) => {
  ...
  const room = Room.find(
    (room) => room.room_id === id && room.booked_status === false
  );
  if (room) {
    let Roomname = Room.find((room) => room.room_id === id);
    Room = Room.map((room) =>
      room.room_id == id ? { ...room, booked_status: true } : room
    );
    RoomBookedCustomers = RoomBookedCustomers.concat([
      { ...req.body, room_id: id },
    ]);
...
app.get("/bookedroom", (req, res) => {
  ...
  BookedRooms = BookedRooms.map((room) => {
    let customerDetail = RoomBookedCustomers.find(
      (bookedroom) => room.room_id === bookedroom.room_id
    );
    return {
...
app.get("/customer/:name", (req, res) => {
  ...
  let BookedCustomers = Customers.filter(
    (customer) => customer.customer_name.toLowerCase() === CusName.toLowerCase()
  );
...
app.post("/cancelbooking/:id", (req, res) => {
  const id = Number(req.params.id);
  const room = Room.find(
    (room) => room.room_id == id && room.booked_status === true
  );
  if (room) {
    Room = Room.map((room) =>
      room.room_id == id ? { ...room, booked_status: false } : room
    );
    RoomBookedCustomers = RoomBookedCustomers.filter(
      (customer) => customer.room_id != id
    );
    res
      .status(201)
      .json({ message: "Room booking status successfully changed " });
  } else {
